...
export class ImageGenerator {
  ...
  async addSpeechBubble(text, direction) {
    try {
      ...
      // Sanitize text: remove special characters, newlines, tabs, etc.
      ...
      const tempTextObj = new FabricText(sanitized, {
        fontSize: fontSize,
      ...
      const avgCharWidth = tempTextObj.width / Math.max(1, sanitized.length);
      ...
      const wrappedLines = this.wrapText(sanitized, targetLineWidth);
